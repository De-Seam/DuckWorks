...
void ThreadManager::AddTask(const SharedPtr<ThreadTask>& inTask, ThreadPriority inPriority)
{
	...
	inTask->mPriority = inPriority;
	{
		...
		mTasks[static_cast<uint64>(inPriority)].Enqueue(inTask);
	}
...
void ThreadManager::WaitUntilPriorityEmpty(ThreadPriority inPriority)
{
	...
	{
		return mTasks[static_cast<uint64>(inPriority)].IsEmpty();
	});
}
	...
	{
		{
			...
			{
				for (int32 i = static_cast<int32>(ThreadPriority::VeryHigh); i >= 0; i--)
				{
					if (!mTasks[i].IsEmpty())
						return true;
				}
...
void ThreadManager::OnTaskCompleted(SharedPtr<ThreadTask>& inTask)
{
	...
	const uint64 index = static_cast<uint64>(inTask->mPriority);
