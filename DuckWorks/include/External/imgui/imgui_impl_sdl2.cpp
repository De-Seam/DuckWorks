...
static void ImGui_ImplSDL2_SetPlatformImeData(ImGuiViewport* viewport, ImGuiPlatformImeData* data)
{
	...
	{
		SDL_Rect r;
		r.x = static_cast<int>(data->InputPos.x - viewport->Pos.x);
		r.y = static_cast<int>(data->InputPos.y - viewport->Pos.y + data->InputLineHeight);
		r.w = 1;
...
static void ImGui_ImplSDL2_UpdateMonitors()
{
	...
	for (int n = 0; n < display_count; n++)
	{
		...
		monitor.MainPos = monitor.WorkPos = ImVec2(static_cast<float>(r.x), static_cast<float>(r.y));
		monitor.MainSize = monitor.WorkSize = ImVec2(static_cast<float>(r.w), static_cast<float>(r.h));
#if SDL_HAS_USABLE_DISPLAY_BOUNDS
		SDL_GetDisplayUsableBounds(n, &r);
		monitor.WorkPos = ImVec2(static_cast<float>(r.x), static_cast<float>(r.y));
		monitor.WorkSize = ImVec2(static_cast<float>(r.w), static_cast<float>(r.h));
#endif
...
#endif
		monitor.PlatformHandle = (void*)static_cast<intptr_t>(n);
...
static void ImGui_ImplSDL2_CreateWindow(ImGuiViewport* viewport)
{
	...
	ImGuiViewport* main_viewport = ImGui::GetMainViewport();
	ImGui_ImplSDL2_ViewportData* main_viewport_data = static_cast<ImGui_ImplSDL2_ViewportData*>(main_viewport->PlatformUserData);
...
#endif
	vd->Window = SDL_CreateWindow("No Title Yet", static_cast<int>(viewport->Pos.x), static_cast<int>(viewport->Pos.y), static_cast<int>(viewport->Size.x),
								static_cast<int>(viewport->Size.y), sdl_flags);
	vd->WindowOwned = true;
	if (use_opengl)
	{
...
{
	if (ImGui_ImplSDL2_ViewportData* vd = static_cast<ImGui_ImplSDL2_ViewportData*>(viewport->PlatformUserData))
	{
