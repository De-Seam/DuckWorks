...
{
	return ImGui::GetCurrentContext() ? static_cast<ImGui_ImplSDLRenderer2_Data*>(ImGui::GetIO().BackendRendererUserData) : nullptr;
}
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			else
			{
				...
				SDL_Rect r = {
					static_cast<int>(clip_min.x), static_cast<int>(clip_min.y), static_cast<int>(clip_max.x - clip_min.x),
					static_cast<int>(clip_max.y - clip_min.y)
				};
				SDL_RenderSetClipRect(bd->SDLRenderer, &r);

				const float* xy = static_cast<const float*>((const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) + offsetof(ImDrawVert, pos)));
				const float* uv = static_cast<const float*>((const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) + offsetof(ImDrawVert, uv)));
#if SDL_VERSION_ATLEAST(2, 0, 19)
				const SDL_Color* color = static_cast<const SDL_Color*>((const void*)((const char*)(vtx_buffer + pcmd->VtxOffset) + offsetof(ImDrawVert, col)));
				// SDL 2.0.19+
#else
